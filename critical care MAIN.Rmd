...
fulldf2 <- left_join(fulldf2, tempdf4, by=c('subject_id', 'hadm_id'))

#combine whiteblood cells with datafram
tempdf4 <- intersect(tempdf2, whiteblood) %>%
  mutate(whiteblood = 1)
tempdf5 <- setdiff(tempdf2, whiteblood) %>%
  mutate(whiteblood = 0)

tempdf6 <- bind_rows(tempdf4, tempdf5)
fulldf2 <- left_join(fulldf2, tempdf6, by=c('subject_id', 'hadm_id'))
fulldf2 <- fulldf2 %>%
  filter(whiteblood == 0) %>%
  select(-whiteblood)
...
fulldf2 <- left_join(fulldf2, sofa, by=c('subject_id', 'hadm_id', 'icustay_id'))
...
fulldf2 <- fulldf2 %>%
  filter(drug_formatted != 'cefepime desensitization')
...
#length of stay vis
ggplot(data=fulldf2, aes(y=los_icu, x=as.factor(drug_formatted))) +
  geom_boxplot() +
  stat_summary(fun.y=mean, geom="point", col="red",size=3)

#mortality vis
ggplot(data=fulldf2, aes(y=mortality, x=as.factor(drug_formatted))) +
  geom_point() +
  stat_summary(fun.y=mean, geom="point", col="red",size=3)

#length of stay t.test
t.test(fulldf2$los_icu[fulldf2$drug_formatted == 'piperacillin-tazobactam'], fulldf2$los_icu[fulldf2$drug_formatted == 'cefepime'], alternative='two.sided', paired=F)

#mortality t.test
t.test(fulldf2$mortality[fulldf2$drug_formatted == 'piperacillin-tazobactam'], fulldf2$mortality[fulldf2$drug_formatted == 'cefepime'], alternative='two.sided', paired=F)

#regressions
set.seed(42)
inTraining <- createDataPartition(fulldf2$mortality, p = .80, list = FALSE)
train <- fulldf2[ inTraining,]
test  <- fulldf2[-inTraining,]

#mortality linear model
model <- glm(mortality ~ (organ_dysfunction + oasis + mech_vent + age + gender +sofa) * drug_formatted, data=train, family=binomial)

#
model3 <- glm(mortality ~ organ_dysfunction + mech_vent + age + gender +sofa + drug_formatted*oasis, data=train, family=binomial)

ggplot(data=)


#2 predictors: oasis and mech_vent, drug choice not relevant

#los_icu model
model2 <- lm(los_icu ~ organ_dysfunction + oasis + mech_vent + age + gender +drug_formatted+ septicshock+ sofa,data=train)
